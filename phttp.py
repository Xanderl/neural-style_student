import http.client, json, calendar, os, time, base64, requests
    ...
    while i < len(conversation) :
       ...
       # print(comment)
        ...
        if(comment['content'] == 'Go!') :
            ...
            t = url.split("/")
            style_img_filename = "style-" + current_time_int + "-" + t[-1]

            r = requests.get(url, allow_redirects=True)
            open(style_img_filename, 'wb').write(r.content)
            ...
            t = url.split("/")
            content_img_filename = "content-" + current_time_int + "-" + t[-1]

            r = requests.get(url, allow_redirects=True)
            open(content_img_filename, 'wb').write(r.content)
